...
contract.only('INXTokenSale', function ([owner, investor, wallet, purchaser, authorized, unauthorized, anotherAuthorized, authorizedTwo, authorizedThree, authorizedFour, authorizedFive]) {
    ...
    beforeEach(async function () {
        ...
        // setup INX contract!!
        this.crowdsale = await INXTokenSale.new(wallet, this.token.address, 1, 2, {from: owner});
    ...
    after(async function () {
        console.log('INXTokenSale Owner', await this.crowdsale.owner());
    ...
    describe('INXTokenSale', function () {
        ...
        describe('construction', function () {
            it('should ensure rate is non-zero', async function () {
                await assertRevert(INXTokenSale.new(wallet, this.token.address, 0, 2, {from: owner}));
            });

            it('should ensure pre-sale rate is non-zero', async function () {
                await assertRevert(INXTokenSale.new(wallet, this.token.address, 1, 0, {from: owner}));
            });
        });
        ...
        describe('contract setup', function () {
            ...
            describe('when the requested account has no token balance', function () {
                it('returns zero', async function () {
                    const balance = await this.crowdsale.tokenBalanceOf(unauthorized);
            ...
            describe('when the requested account has no wei balance', function () {
                it('returns zero', async function () {
                    const balance = await this.crowdsale.weiBalanceOf(unauthorized);
                    balance.should.be.bignumber.equal('0');
                });
            });
        });

        describe('pausable actions in escrow', function () {
            ...
            it('should not allow commitment when paused', async function () {
                ...
                await assertRevert(this.crowdsale.commitToBuyTokens({value: this.minContribution, from: unauthorized}));
            });
